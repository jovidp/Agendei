...
function migrarVinculos(PDO $db): array
{
    ...
    if (!$tabelaExiste('vinculos')) {
        if ($ehPostgres) {
            // No PostgreSQL o nome de um indice e unico no esquema inteiro, e
            // uk_estabelecimento_login ainda pertence a usuarios neste ponto:
            // a unicidade do login nasce com nome provisorio e e renomeada no
            // passo 7, depois que usuarios perde a dela.
    ...
    foreach ($perfis as $tabela => $tipo) {
        ...
        // indice comum entra antes, senao o banco recusa a remocao. O nome e o
        // que o MySQL daria ao indice implicito da chave, como em banco.sql.
        if (!$ehPostgres && !$indiceExiste($tabela, "fk_{$tabela}_usuario") && !$indiceExiste($tabela, "idx_{$tabela}_usuario")) {
            $db->exec("CREATE INDEX fk_{$tabela}_usuario ON $tabela (id_usuario)");
        }
        ...
        // O esquema PostgreSQL ja traz este par de colunas como idx_*_tenant_usuario.
        if (!$indiceExiste($tabela, "idx_{$tabela}_pessoa") && !$indiceExiste($tabela, "idx_{$tabela}_tenant_usuario")) {
            $db->exec("CREATE INDEX idx_{$tabela}_pessoa ON $tabela (id_estabelecimento, id_usuario)");
        }
        // No MySQL a chave composta logo abaixo cria o proprio indice, como em banco.sql.
        if ($ehPostgres && !$indiceExiste($tabela, "idx_{$tabela}_tenant_vinculo")) {
    ...
    // ---------------------------------------------------------------------
    // 7. A unicidade do login fica com o mesmo nome dos esquemas
    //    (uk_estabelecimento_login), agora que usuarios nao usa mais esse nome.
    // ---------------------------------------------------------------------
    if ($ehPostgres) {
        if ($constraintExiste('vinculos', 'uk_vinculos_login') && !$indiceExiste('usuarios', 'uk_estabelecimento_login')
            && !$constraintExiste('vinculos', 'uk_estabelecimento_login')) {
            $db->exec('ALTER TABLE vinculos RENAME CONSTRAINT uk_vinculos_login TO uk_estabelecimento_login');
            $feito[] = 'unicidade do login em vinculos renomeada para uk_estabelecimento_login';
        }
    } elseif ($indiceExiste('vinculos', 'uk_vinculos_login') && !$indiceExiste('vinculos', 'uk_estabelecimento_login')) {
        // MariaDB 10.4 nao renomeia indice: derruba e recria com o nome definitivo.
        $db->exec('ALTER TABLE vinculos DROP INDEX uk_vinculos_login, ADD UNIQUE KEY uk_estabelecimento_login (id_estabelecimento, login)');
        $feito[] = 'unicidade do login em vinculos recriada como uk_estabelecimento_login';
    }
