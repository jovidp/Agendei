...
(function () {
  ...
  function carregarProfissionais() {
    ...
    buscarJson(
      "api/profissionais.php?id_servico=" +
        encodeURIComponent(campoServico.value),
    )
      .then(function (dados) {
        ...
        if ((dados.profissionais || []).length === 0) {
          limparSelect(
            campoProfissional,
            "Nenhum profissional executa este servico",
          );
        }
      })
      .catch(function () {
  ...
  function carregarHorarios() {
    var idProfissional =
      profissionalFixo || (campoProfissional ? campoProfissional.value : "");
    ...
    buscarJson(
      "api/horarios.php?id_profissional=" +
        encodeURIComponent(idProfissional) +
        "&id_servico=" +
        encodeURIComponent(campoServico.value) +
        "&data=" +
        encodeURIComponent(campoData.value),
    )
      .then(function (dados) {
        ...
        if (horarios.length === 0) {
          ...
          if (avisoHorarios) {
            avisoHorarios.textContent =
              "O profissional nao possui horarios livres nesta data. Verifique o expediente e os bloqueios.";
