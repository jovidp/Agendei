    ...
    formulario.addEventListener("submit", function (evento) {
      ...
      if (nascimento !== "") {
        ...
        var idadeValida =
          !isNaN(data.getTime()) &&
          data < hoje &&
          hoje.getFullYear() - data.getFullYear() < 120;
        valido =
          verificar(
            formulario,
            "data_nascimento",
    ...
    formulario.addEventListener("submit", function (evento) {
      ...
      valido =
        verificar(
          formulario,
          "confirmar_senha",
          valor(formulario, "confirmar_senha") === nova && nova !== "",
          "As senhas nao conferem.",
        ) && valido;
    ...
    formulario.addEventListener("submit", function (evento) {
      ...
      valido =
        verificar(
          formulario,
          "nome",
          valor(formulario, "nome").length >= 5,
          "Informe seu nome completo.",
        ) && valido;
      valido =
        verificar(
      ...
      if (campo(formulario, "cpf") && valor(formulario, "cpf") !== "") {
        valido =
          verificar(
            formulario,
            "cpf",
            Agendei.validarCpf(valor(formulario, "cpf")),
            "Informe um CPF valido.",
          ) && valido;
      }
