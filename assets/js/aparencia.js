...
(function () {
    ...
    var logoNova = '';
    var versaoArquivo = 0;
    function rgb(cor) { return [1, 3, 5].map(function (i) { return parseInt(cor.slice(i, i + 2), 16); }); }
    function mistura(cor, alvo, peso) { return '#' + rgb(cor).map(function (v) { return Math.round(v * (1 - peso) + alvo * peso).toString(16).padStart(2, '0'); }).join(''); }
    function texto(cor) {
        var c = rgb(cor).map(function (v) { v /= 255; return v <= .04045 ? v / 12.92 : Math.pow((v + .055) / 1.055, 2.4); });
        return c[0] * .2126 + c[1] * .7152 + c[2] * .0722 > .179 ? '#000000' : '#FFFFFF';
    }
    function atualizar() {
        ['primaria', 'secundaria'].forEach(function (nome) {
            var cor = form.elements['cor_' + nome].value;
            previa.style.setProperty('--cor-' + nome, cor);
            previa.style.setProperty('--cor-' + nome + '-escura', mistura(cor, 0, .25));
            previa.style.setProperty('--cor-' + nome + '-clara', mistura(cor, 255, .9));
            previa.style.setProperty('--sobre-' + nome, texto(cor));
        });
        previa.style.setProperty('--fundo', form.elements.cor_fundo.value);
        previa.style.setProperty('--fonte-interface', form.elements.fonte.selectedOptions[0].dataset.familia);
    ...
    form.elements.remover_logo.addEventListener('change', function () {
        if (this.checked) { versaoArquivo++; logoNova = ''; form.elements.logo.value = ''; }
        atualizar();
    });
    form.elements.logo.addEventListener('change', function () {
        ...
        if (!arquivo) { atualizar(); return; }
        if (arquivo.size > 512 * 1024 || !['image/png', 'image/jpeg', 'image/webp'].includes(arquivo.type)) {
    ...
    document.getElementById('restaurarTema').addEventListener('click', function () {
